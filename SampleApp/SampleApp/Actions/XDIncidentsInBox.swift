...
class XDIncidentsInBox: SampleAction {
    required init () {}
    ...
    func performAction(core: InrixCore, values: [String : ParamValue], completionHandler: @escaping ActionCompletionHandler) {
        ...
        let options = IXCLXDIncidentsInBoxOptions(corner1: region.sw, corner2: region.ne)

        core.incidentsController.requestXDIncidentsInBox(with: options, incidentsFilter: filter) {
